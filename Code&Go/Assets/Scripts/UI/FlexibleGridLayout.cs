...
;
        ...
        columnCount++;
        if (fitType == FitType.FixedColumns)
            rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, (cellSize.y * rowCount) + (spacing.y * rowCount) + padding.top + padding.bottom);

        if (fitType == FitType.FixedRows)
            rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, (cellSize.x * columnCount) + (spacing.x * columnCount) + padding.left + padding.right);
    }
